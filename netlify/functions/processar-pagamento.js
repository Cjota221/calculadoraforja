...
const DESCRIPTION = 'Forja Vitalício';
...
exports.handler = async (event) => {
  ...
  const webhookUrl = `${process.env.NEXT_PUBLIC_URL}/.netlify/functions/webhook-mercadopago`;
  ...
  if (method === 'card') {
    ...
    if (!cardNumber || !expirationMonth || !expirationYear || !cvv || !cardholderName || !cpf) {
      return { statusCode: 400, headers: cors, body: JSON.stringify({ error: 'Dados do cartão incompletos.' }) };
    }
    ...
    try {
      ...
      if (!paymentMethod) {
        return { statusCode: 400, headers: cors, body: JSON.stringify({ error: 'Bandeira do cartão não identificada.' }) };
      }
      ...
      if (payment.status === 'rejected') {
        ...
        return {
          ...
          body: JSON.stringify({ error: motivos[detalhe] || 'Pagamento recusado. Tente outro cartão.' })
        };
      }
  ...
  return { statusCode: 400, headers: cors, body: JSON.stringify({ error: 'Método inválido.' }) };
};
